...
snpTable$logP <- -log10(snpTable$pval)
...
phenotypedGenos <- phenotype$Genotype
...
snpTable$clump <- NA
snpTable <- snpTable[order(snpTable$pval,as.numeric(snpTable$chrom),as.numeric(snpTable$pos)),]
...
######Filter so each clump contains only one representative from each trait, select the one with the highest p-value####
filterTable <- snpTable %>% group_by(trait,clump) %>% top_n(n=-1,wt=pval)
