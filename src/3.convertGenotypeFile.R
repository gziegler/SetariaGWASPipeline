...
geno <- fread(paste0("../data/genotype/2.from12.setaria.maf0.1.maxMissing0.1.allLines.012"),header=FALSE,sep="\t")
indv <- read.table(paste0("../data/genotype/2.from12.setaria.maf0.1.maxMissing0.1.allLines.012.indv"),sep="\t",header=FALSE,stringsAsFactors = FALSE)
snpInfo <- fread(paste0("../data/genotype/2.from12.setaria.maf0.1.maxMissing0.1.allLines.012.pos"),sep="\t",header=FALSE,stringsAsFactors = FALSE)
...
geno <- geno[-1,]
message("Finished reading genotype files. Changing missing (-1) to NA.")
###Change -1 values to missing
for(j in seq_len(ncol(geno))){
  set(geno,which(geno[[j]]==-1),j,NA)
}
message("There are ",nrow(geno)," snps.")
...
message("Finished calculating het calls. Now calculating missing and minor allele frequency.")
...
numDiscard <- length(which(alleleTable$discard==TRUE))
message("Finished calculating MAF: ",numDiscard," SNPs being discarded.")
...
message("SNPs discarded and data.table file saved. Converting to matrix.")
...
message("Matrix format table saved. Finished.")
